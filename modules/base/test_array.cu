...
typedef Array<uchar> array_u;
void median_3d(unsigned char* deviceSrc, unsigned char* deviceDst, int dimx, int dimy, int dimz, int radius, int halo, cudaStream_t stream);
...
int main (int argc, char **argv)
{
	...
	array_u src(dimx, dimy, dimz);
	array_u dst(dimx, dimy, dimz);
	
	src.ReadFile("../3_130201zf142_60nmpx_512test2_tn15278to15789_ElasticAligned_downsample_from_20nm_crop.raw", dimxyz*sizeof(uchar));
	
	median_3d(src.dPtr(), dst.dPtr(), dimx, dimy, dimz, 3, 3, 0);
	
	dst.SaveFile("../3_130201zf142_60nmpx_512test2_tn15278to15789_ElasticAligned_downsample_from_20nm_crop_median3.raw", dimxyz*sizeof(uchar));
	return EXIT_SUCCESS;
}
...
__global__ 
void __median_3d(unsigned char* deviceSrc, unsigned char* deviceDst, int dimx, int dimy, int dimz, int radius, int halo);
...
void __median_3d(unsigned char* deviceSrc, unsigned char* deviceDst, int dimx, int dimy, int dimz, int radius, int halo)
{
    ...
    unsigned char result = sharedMemSrc[at(threadIdx.x + halo, threadIdx.y + halo, threadIdx.z + halo, sharedMemDim.x, sharedMemDim.y, sharedMemDim.z)];   
	 // # Viola's method
		// minval = 0
		// maxval = 255
	...
	for(int trial=0; trial<8; trial++)
	{
		...
		for(int z=threadIdx.z+halo-radius; z<=threadIdx.z+halo+radius; z++)
		{
			for(int y=threadIdx.y+halo-radius; y<=threadIdx.y+halo+radius; y++)
			{
				...
				{
					val = (float)sharedMemSrc[at(x, y, z, sharedMemDim.x, sharedMemDim.y, sharedMemDim.z)];   
					if(val>pivot)	
						count++;
	...
	result = (unsigned char)floorf(pivot);
